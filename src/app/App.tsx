...
import { AutomationV2View } from "./components/automation-v2";
...
export default function App() {
  ...
  // Org hierarchy — two levels: super orgs + child orgs
  const [allTenants, setAllTenants] = useState<Tenant[]>(INITIAL_TENANTS);
  const superOrgs = useMemo(() => allTenants.filter(t => t.orgRole === "super"), [allTenants]);
  const childOrgs = useMemo(() => allTenants.filter(t => t.parentOrgId === activeTenant.id), [allTenants, activeTenant.id]);
  const [viewingOrgId, setViewingOrgId] = useState<string | null>(null); // null = super org (HQ) view
  ...
  const navSections: { label: string; items: { id: string; name: string; icon: any; badge?: number }[] }[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: "campaigns",       name: "Campaigns",       icon: ClipboardList },
    ...
    <div className="flex h-screen bg-background text-foreground font-sans antialiased overflow-hidden">
      ...
      >
        ...
        <div className="relative px-3 pt-3 pb-2">
          <button
            ...
            className={cn(
              ...
              isOrgSwitcherOpen
                ...
                : "bg-white dark:bg-white/10 hover:bg-white/90 dark:hover:bg-white/15"
            )}
          >
          ...
          <button
            onClick={() => setIsMobileSidebarOpen(false)}
            className="lg:hidden absolute top-3 right-3 p-1.5 text-muted-foreground hover:text-foreground transition-colors"
          ...
          {/* Org dropdown */}
          <AnimatePresence>
            {isOrgSwitcherOpen && (!isSidebarCollapsed || isMobileSidebarOpen) && (
              <>
                ...
                >
                  <div className="py-1.5 max-h-[360px] overflow-y-auto">
                    {superOrgs.map((org, i) => {
                      const isSelected = activeTenant.id === org.id;
                      const colors = ["bg-primary", "bg-violet-500", "bg-cyan-500", "bg-amber-500", "bg-emerald-500", "bg-rose-500"];
                      return (
                        <button
                          key={org.id}
                          onClick={() => { setActiveTenant(org); setViewingOrgId(null); setIsOrgSwitcherOpen(false); }}
                        ...
                        >
                          <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center shrink-0 text-white shadow-sm", colors[i % colors.length])}>
      ...
      {/* Main Content */}
      <main id="main-content" className="flex-1 flex flex-col min-w-0 h-full relative z-10">
      ...
      <Modal isOpen={isCreateOrgOpen} onClose={() => { setIsCreateOrgOpen(false); setNewOrgForm({ name: "", description: "" }); }} title="Create New Organization" size="md">
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">Add a new super organization to your workspace</p>
          <div className="space-y-2">
            <Label htmlFor="org-name">Organization Name</Label>
            <Input id="org-name" value={newOrgForm.name} onChange={(e) => setNewOrgForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. New Hope Ministry" autoFocus />
          </div>
          <div className="space-y-2">
            <Label htmlFor="org-desc">Description</Label>
            <Textarea id="org-desc" value={newOrgForm.description} onChange={(e) => setNewOrgForm(p => ({ ...p, description: e.target.value }))} placeholder="Brief description of this organization..." rows={3} />
          </div>
          <div className="flex items-center justify-end gap-2 pt-2">
            <Button variant="outline" size="sm" onClick={() => { setIsCreateOrgOpen(false); setNewOrgForm({ name: "", description: "" }); }}>Cancel</Button>
            <Button size="sm" disabled={!newOrgForm.name.trim()} onClick={() => {
              if (!newOrgForm.name.trim()) return;
              const newOrg: Tenant = {
                id: `tenant-${Date.now()}`, name: newOrgForm.name.trim(), industry: "Ministry", plan: "free",
                createdAt: new Date().toISOString().slice(0, 10), stats: { contacts: 0, messages: 0, activeUsers: 0 },
                orgRole: "super", parentOrgId: null, orgStatus: "active", region: "", description: newOrgForm.description.trim(),
              };
              setAllTenants(prev => [...prev, newOrg]);
