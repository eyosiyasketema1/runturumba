...
import {
  ...
  LayoutTemplate, Layers, ArrowDownRight, ArrowUpRight, Workflow
} from "lucide-react";
...
type WorkflowMode = "basic" | "sequence" | "journey";
...
const NODE_CATALOG: { section: string; description: string; icon: any; items: NodeCatalogItem[] }[] = [
  ...
  {
    section: "Actions",
    description: "Do something in your workflow",
    icon: Play,
    items: [
  ...
  },
  {
    ...
    description: "Branch, filter, delay, or loop your workflow",
    icon: GitBranch,
    items: [
...
const SAMPLE_WORKFLOWS: WorkflowDraft[] = [
  {
    id: "wf-1", name: "Welcome New Contacts", description: "Send a welcome message when a contact is created",
    mode: "basic", enabled: true, createdAt: "2025-05-10T08:00:00Z", runs: 342,
    nodes: [
  ...
  },
  {
    ...
    mode: "basic", enabled: true, createdAt: "2025-05-08T10:30:00Z", runs: 1205,
    nodes: [
  ...
  },
  {
    ...
    mode: "sequence", enabled: true, createdAt: "2025-04-20T14:00:00Z", runs: 89,
    nodes: [
  ...
  },
  {
    ...
    mode: "journey", enabled: true, createdAt: "2025-04-15T09:00:00Z", runs: 213,
    nodes: [
  ...
  },
  {
    ...
    mode: "basic", enabled: true, createdAt: "2025-05-15T11:00:00Z", runs: 56,
    nodes: [
  ...
  },
  {
    ...
    mode: "sequence", enabled: false, createdAt: "2025-05-01T09:00:00Z", runs: 0,
    nodes: [
...
const getModeInfo = (mode: WorkflowMode) => {
  switch (mode) {
...
// ============================================================================
// SVG CONNECTION LINES — draws bezier curves between nodes
// ============================================================================
  ...
  return (
    <svg className="absolute inset-0 pointer-events-none" style={{ overflow: "visible" }}>
      ...
      {connections.map(conn => {
        ...
        const strokeColor = conn.type === "true" ? "#22c55e"
          : conn.type === "false" ? "#ef4444"
          : "#94a3b8";
        const markerId = conn.type === "true" ? "arrow-true"
          : conn.type === "false" ? "arrow-false"
          : "arrow-default";
...
// ============================================================================
// CANVAS NODE — the visual card on the canvas
// ============================================================================

const CanvasNode = ({
  node, isSelected, onClick, onDelete, onAddAfter,
}: {
  node: FlowNode;
  isSelected: boolean;
  onClick: () => void;
  ...
  return (
    <div
      ...
      style={{ left: pos.px, top: pos.py, width: NODE_WIDTH, height: NODE_HEIGHT }}
    >
      {/* The card */}
      <div
        onClick={onClick}
        className={cn(
          "w-full h-full rounded-xl border-2 cursor-pointer transition-all duration-200",
          "bg-card hover:shadow-lg hover:shadow-primary/5",
          isSelected
            ? `border-primary shadow-lg shadow-primary/10 ring-2 ${colors.ring}`
...
const NodePickerPanel = ({
  isOpen, onClose, onSelectNode, title, mode,
}: {
  isOpen: boolean; onClose: () => void;
  onSelectNode: (item: NodeCatalogItem) => void;
  title: string;
  mode: WorkflowMode;
}) => {
  ...
  const filteredCatalog = useMemo(() => {
    ...
    // For basic mode, hide flow control except filter
    if (mode === "basic") {
      catalog = catalog.map(s => s.section === "Flow Control"
        ? { ...s, items: s.items.filter(i => i.category === "filter") }
        : s
        ...
        {filteredCatalog.map(section => {
          ...
          return (
            <div key={section.section}>
              <button
                onClick={() => setExpandedSection(isExpanded && !search ? null : section.section)}
                  ...
                  {section.items.map(item => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-5 pl-8 py-2.5 hover:bg-primary/5 transition-colors group"
                      >
...
const NodeInspector = ({
  node, onUpdate, onClose, onDelete, isJourney,
}: {
  node: FlowNode;
      ...
      <div className="sticky top-0 bg-card z-10 border-b border-border">
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-2.5">
            <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center", node.iconBg)}>
      ...
      <div className="p-5 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            {node.category === "ai_respond" && (
              <div className="flex items-center gap-2 pt-1 bg-muted/50 rounded-lg px-3 py-2">
                ...
                <Label htmlFor="ai-personalize" className="text-xs text-muted-foreground flex items-center gap-1.5 cursor-pointer">
                  <Sparkles className="w-3 h-3 text-pink-500" /> AI Personalize responses
                </Label>
              </div>
            )}
          </div>
        ...
        {node.category === "wait" && (
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              <Input
                type="number"
                value={node.config.hours || 24}
                onChange={(e) => onUpdate({ config: { ...node.config, hours: parseInt(e.target.value) || 0 } })}
              ...
              <select
                ...
                className="h-9 px-3 rounded-md border border-input bg-background text-sm flex-1"
              >
        ...
        {(node.category === "add_tag" || node.category === "remove_tag" || node.category === "tag_added") && (
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => onUpdate({ config: { ...node.config, tag: e.target.value } })}
              className="h-9 text-sm"
              placeholder="e.g. new-believer, vip"
            />
          </div>
        )}
        ...
        {node.category === "if_else" && (
          <div className="space-y-3">
            ...
            <select
              value={node.config.condition || "has_replied"}
            ...
            >
              <option value="has_replied">Contact has replied</option>
              <option value="has_tag">Contact has tag</option>
              <option value="message_contains">Message contains keyword</option>
              <option value="milestone_count">Milestone count is</option>
              <option value="days_since_last">Days since last message</option>
              <option value="in_group">Contact is in group</option>
            </select>
            {isJourney && (
              <div className="bg-orange-50 dark:bg-orange-500/10 rounded-lg p-3 border border-orange-200 dark:border-orange-500/20">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-orange-600 dark:text-orange-400/80 mt-0.5">
                      TRUE path goes up, FALSE path goes down. Add nodes after each branch.
                    </p>
                  </div>
                </div>
              </div>
        ...
        {node.category === "message_received" && (
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => onUpdate({ config: { ...node.config, keyword: e.target.value } })}
              className="h-9 text-sm"
              placeholder="e.g. help, info, start"
            />
        ...
        {node.category === "schedule" && (
          <div className="space-y-1.5">
            ...
            <select
              value={node.config.interval || "daily"}
              onChange={(e) => onUpdate({ config: { ...node.config, interval: e.target.value } })}
              className="h-9 px-3 rounded-md border border-input bg-background text-sm w-full"
            >
        ...
        {/* Position (journey mode) */}
        {isJourney && (
          <div className="space-y-1.5">
            <Label className="text-xs font-semibold text-foreground">Grid Position</Label>
            <div className="flex items-center gap-2">
              <div className="flex-1">
                <p className="text-[10px] text-muted-foreground mb-1">Column (X)</p>
                <Input
                  type="number"
                  value={node.position.x}
                  onChange={(e) => onUpdate({ position: { ...node.position, x: parseFloat(e.target.value) || 0 } })}
                  className="h-8 text-xs"
                  min={0}
                />
              </div>
              <div className="flex-1">
                <p className="text-[10px] text-muted-foreground mb-1">Row (Y)</p>
                <Input
                  type="number"
                  value={node.position.y}
                  onChange={(e) => onUpdate({ position: { ...node.position, y: parseFloat(e.target.value) || 0 } })}
                  className="h-8 text-xs"
                  min={0}
                  step={0.5}
                />
        ...
        <div className="pt-4 border-t border-border">
          <Button variant="outline" size="sm" className="w-full text-destructive hover:text-destructive hover:bg-destructive/5" onClick={onDelete}>
...
// ============================================================================
// WORKFLOW CANVAS — visual editor with SVG connections
// ============================================================================

const WorkflowCanvas = ({
  workflow, onBack, onSave, onUpdateWorkflow,
}: {
  workflow: WorkflowDraft;
  onBack: () => void;
  onSave: (wf: WorkflowDraft) => void;
  onUpdateWorkflow: (wf: WorkflowDraft) => void;
}) => {
  ...
  const modeInfo = getModeInfo(workflow.mode);

  // Calculate canvas bounds
  const canvasBounds = useMemo(() => {
    if (workflow.nodes.length === 0) return { width: 800, height: 400 };
    let maxX = 0, maxY = 0;
  ...
  const addNode = useCallback((item: NodeCatalogItem) => {
    const isJourney = workflow.mode === "journey";
    let posX = 0, posY = isJourney ? 1 : 0;
    const newConnections = [...workflow.connections];

    if (insertAfterNodeId) {
      const afterNode = workflow.nodes.find(n => n.id === insertAfterNodeId);
      if (afterNode) {
        posX = afterNode.position.x + 1;
        posY = afterNode.position.y;

        // Shift any nodes at or after this position
        const updatedNodes = workflow.nodes.map(n => {
          if (n.position.x >= posX && Math.abs(n.position.y - posY) < 0.5) {
            return { ...n, position: { ...n.position, x: n.position.x + 1 } };
          }
          return n;
        });
        ...
        const newNode: FlowNode = {
          id: newId, type: item.type, category: item.category, label: item.label,
          description: item.description, icon: item.icon, iconColor: item.iconColor,
          iconBg: item.iconBg, config: {}, position: { x: posX, y: posY },
        };

        // Re-wire: find connection from afterNode, splice in new node
        const existingConn = workflow.connections.find(c => c.from === insertAfterNodeId);
        let finalConnections = workflow.connections.filter(c => c.from !== insertAfterNodeId || c === existingConn);

        // Add connection from afterNode to new node
        finalConnections = [...finalConnections.filter(c => c.from !== insertAfterNodeId),
          { id: `c-${Date.now()}-a`, from: insertAfterNodeId, to: newId }
        ];
        // If there was an existing next node, connect new node to it
        if (existingConn) {
          finalConnections.push({ id: `c-${Date.now()}-b`, from: newId, to: existingConn.to });
        }

        onUpdateWorkflow({ ...workflow, nodes: [...updatedNodes, newNode], connections: finalConnections });
    ...
    if (workflow.nodes.length > 0) {
      ...
      posX = maxX + 1;
      posY = workflow.nodes[0]?.position.y ?? (isJourney ? 1 : 0);
    }
    ...
    // Auto-connect from last node in the main path
    const nodesAtMaxX = workflow.nodes.filter(n => n.position.x === Math.max(...workflow.nodes.map(n2 => n2.position.x)));
    const lastNode = nodesAtMaxX.length > 0 ? nodesAtMaxX[0] : null;
    const newConns = [...workflow.connections];
    if (lastNode) {
      newConns.push({ id: `c-${Date.now()}`, from: lastNode.id, to: newId });
    }

    // For journey IF/ELSE: auto-create true/false branch placeholders
    if (isJourney && item.category === "if_else") {
      // We don't auto-create branch nodes — user adds them via the "+" button
    }

    onUpdateWorkflow({ ...workflow, nodes: [...workflow.nodes, newNode], connections: newConns });
    setIsNodePickerOpen(false);
    setInsertAfterNodeId(null);
    setSelectedNodeId(newId);
    toast.success(`Added "${item.label}"`);
  }, [workflow, insertAfterNodeId, onUpdateWorkflow]);

  const deleteNode = (nodeId: string) => {
    // Remove node and all connections involving it
    const newNodes = workflow.nodes.filter(n => n.id !== nodeId);
    const newConns = workflow.connections.filter(c => c.from !== nodeId && c.to !== nodeId);

    // Try to re-link: if A→deleted→B, create A→B
    ...
    onUpdateWorkflow({ ...workflow, nodes: newNodes, connections: newConns });
  ...
  return (
    <div className="h-[calc(100vh-64px)] flex flex-col bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border bg-card shrink-0">
        <div className="flex items-center gap-3">
          ...
          {isEditingName ? (
            <Input
              ...
              onBlur={() => { setIsEditingName(false); onUpdateWorkflow({ ...workflow, name: wfName }); }}
              onKeyDown={(e) => { if (e.key === "Enter") { setIsEditingName(false); onUpdateWorkflow({ ...workflow, name: wfName }); } }}
              className="h-8 text-sm font-semibold w-[260px]"
              autoFocus
            />
          ) : (
            <button onClick={() => setIsEditingName(true)} className="flex items-center gap-2 group">
              <h2 className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{workflow.name}</h2>
        ...
        <div className="flex items-center gap-2">
          <Badge variant={workflow.enabled ? "default" : "secondary"} className={cn("text-xs", workflow.enabled && "bg-emerald-100 text-emerald-700 border-emerald-200")}>
            {workflow.enabled ? "Active" : "Draft"}
          </Badge>
          <div className="w-px h-6 bg-border" />
          <Button variant="outline" size="sm" onClick={() => { onSave(workflow); toast.success("Saved!"); }}>
            <Save className="w-3.5 h-3.5" /> Save
          </Button>
          <Button size="sm" onClick={() => { onSave({ ...workflow, enabled: true }); toast.success("Published!"); onBack(); }}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          ref={canvasRef}
          className="flex-1 overflow-auto relative bg-[radial-gradient(circle,_hsl(var(--muted-foreground)/0.08)_1px,_transparent_1px)] bg-[length:24px_24px]"
        ...
        >
          ...
          <div className="absolute bottom-4 left-4 flex items-center gap-1 bg-card border border-border rounded-lg p-1 z-20 shadow-sm">
            <button onClick={() => setZoom(1)} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground text-xs font-mono w-10 text-center">
              {Math.round(zoom * 100)}%
            </button>
          ...
          {/* Mode hint */}
          {workflow.mode === "journey" && workflow.nodes.length === 0 && (
          ...
          >
            {/* SVG connections */}
            <ConnectionLines nodes={workflow.nodes} connections={workflow.connections} />
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="flex items-center gap-8">
                  <button
                    ...
                    className="w-[160px] h-[160px] border-2 border-dashed border-border rounded-2xl flex flex-col items-center justify-center gap-3 hover:border-primary/50 hover:bg-primary/5 transition-all group"
                  >
                  ...
                  <button
                    onClick={() => {
                      const trigger = NODE_CATALOG[0].items[5]; // manual trigger
                      addNode(trigger);
                    }}
                    className="w-[160px] h-[160px] border-2 border-dashed border-border rounded-2xl flex flex-col items-center justify-center gap-3 hover:border-primary/50 hover:bg-primary/5 transition-all group"
                  >
            ...
            {workflow.nodes.map(node => (
              <CanvasNode
                ...
                onAddAfter={() => handleOpenPicker(node.id)}
              />
            ))}

            {/* Global "+" button when nodes exist but none selected */}
            {workflow.nodes.length > 0 && !isNodePickerOpen && !selectedNode && (
              <div className="absolute bottom-6 right-6 z-10">
                <button
                  onClick={() => handleOpenPicker()}
                  className="w-12 h-12 rounded-full bg-primary text-primary-foreground shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center"
                >
        ...
        {/* Right panel — node picker OR inspector */}
        {isNodePickerOpen && (
          <NodePickerPanel
            isOpen={isNodePickerOpen}
            onClose={() => { setIsNodePickerOpen(false); setInsertAfterNodeId(null); }}
            onSelectNode={addNode}
            title={insertAfterNodeId ? "Add next step" : "What happens first?"}
            mode={workflow.mode}
          />
        )}
        {selectedNode && !isNodePickerOpen && (
          <NodeInspector
            ...
            isJourney={workflow.mode === "journey"}
          />
        )}
      </div>
    </div>
...
// ============================================================================
// NEW WORKFLOW MODAL — lets user pick mode
// ============================================================================

const NewWorkflowPicker = ({
  isOpen, onClose, onCreate,
}: {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (mode: WorkflowMode, name: string) => void;
}) => {
  const [selectedMode, setSelectedMode] = useState<WorkflowMode>("basic");
  const [name, setName] = useState("");

  if (!isOpen) return null;

  const modes: { mode: WorkflowMode; icon: any; title: string; desc: string; example: string; color: string; bg: string }[] = [
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg mx-4 animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
              ...
              {modes.map(m => {
                ...
                return (
                  ...
                  >
                    ...
                    {selectedMode === m.mode && (
                      <CheckCircle2 className={cn("w-5 h-5 shrink-0 mt-0.5", m.color)} />
                    )}
                  </button>
                );
              })}
        ...
        <div className="p-6 border-t border-border flex items-center justify-end gap-3">
          ...
          <Button onClick={() => { onCreate(selectedMode, name || "My workflow"); onClose(); }}>
...
// ============================================================================
// MAIN VIEW — WORKFLOW LIST
// ============================================================================

export const AutomationV2View = () => {
  const [workflows, setWorkflows] = useState<WorkflowDraft[]>(SAMPLE_WORKFLOWS);
  const [searchQuery, setSearchQuery] = useState("");
  const [filterMode, setFilterMode] = useState<"all" | WorkflowMode>("all");
  const [filterStatus, setFilterStatus] = useState<"all" | "active" | "draft">("all");
  const [editingWorkflow, setEditingWorkflow] = useState<WorkflowDraft | null>(null);
  const [showNewModal, setShowNewModal] = useState(false);

  // Stats
  const totalActive = workflows.filter(w => w.enabled).length;
  const totalRuns = workflows.reduce((s, w) => s + w.runs, 0);
  const totalJourneys = workflows.filter(w => w.mode === "journey").length;

  const filtered = useMemo(() => {
    return workflows.filter(w => {
      const q = searchQuery.toLowerCase();
      const matchesSearch = !q || w.name.toLowerCase().includes(q) || w.description.toLowerCase().includes(q);
      const matchesMode = filterMode === "all" || w.mode === filterMode;
      const matchesStatus = filterStatus === "all"
        || (filterStatus === "active" && w.enabled)
        || (filterStatus === "draft" && !w.enabled);
      return matchesSearch && matchesMode && matchesStatus;
    });
  }, [workflows, searchQuery, filterMode, filterStatus]);

  const handleCreate = (mode: WorkflowMode, name: string) => {
    const wf: WorkflowDraft = {
      id: `wf-${Date.now()}`, name, description: "", nodes: [], connections: [],
      enabled: false, createdAt: new Date().toISOString(), runs: 0, mode,
    };
    setWorkflows(prev => [...prev, wf]);
    setEditingWorkflow(wf);
  };

  const handleSave = (wf: WorkflowDraft) => {
  ...
  const handleDuplicate = (wf: WorkflowDraft) => {
    const copy: WorkflowDraft = { ...wf, id: `wf-${Date.now()}`, name: `${wf.name} (Copy)`, enabled: false, runs: 0, connections: [...wf.connections] };
  ...
  // Full-page editor
  if (editingWorkflow) {
    const currentWf = workflows.find(w => w.id === editingWorkflow.id) || editingWorkflow;
    return (
      <WorkflowCanvas
        workflow={currentWf}
    ...
    <div className="space-y-5 p-6 animate-in fade-in duration-500">
      ...
      <header className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Build discipleship workflows — Basic, Sequence, and Journey modes</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          { label: "Total", value: workflows.length, icon: Zap, color: "text-primary" },
          ...
          { label: "Drafts", value: workflows.filter(w => !w.enabled).length, icon: Clock, color: "text-amber-600" },
        ].map(stat => (
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <Button className="mt-5" onClick={() => setShowNewModal(true)}>
            <Plus className="w-4 h-4 mr-1.5" /> New Workflow
          </Button>
        </div>
      ) : (
        <div className="grid gap-3">
          {filtered.map(wf => {
            const mInfo = getModeInfo(wf.mode);
            const ModeIcon = mInfo.icon;
            return (
              <div
                key={wf.id}
                className="bg-card border border-border rounded-xl px-5 py-4 hover:border-primary/30 hover:shadow-sm transition-all group cursor-pointer"
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className={cn(
                    ...
                    wf.enabled ? "bg-emerald-50 dark:bg-emerald-500/10" : "bg-muted"
                  )}>
                    <ModeIcon className={cn("w-5 h-5", wf.enabled ? mInfo.color : "text-muted-foreground")} />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className={cn("text-[10px] shrink-0", mInfo.bg, mInfo.color, mInfo.border)}>
                        {mInfo.label}
                      </Badge>
                    </div>
                    {wf.description && (
                  ...
                  <div className="hidden sm:flex items-center gap-1 shrink-0">
                    {wf.nodes.slice(0, 5).map((node, i) => {
                      const NIcon = node.icon;
                      return (
                        <React.Fragment key={node.id}>
                          {i > 0 && <div className="w-2.5 h-[1.5px] bg-border" />}
                          <div className={cn("w-7 h-7 rounded-md flex items-center justify-center", node.iconBg)} title={node.label}>
                            <NIcon className={cn("w-3.5 h-3.5", node.iconColor)} />
                          </div>
                        </React.Fragment>
                      );
                    })}
                    {wf.nodes.length > 5 && (
                      <span className="text-[10px] text-muted-foreground ml-1">+{wf.nodes.length - 5}</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    <div className="text-right hidden md:block">
                      <p className="text-xs font-medium text-foreground tabular-nums">{wf.runs.toLocaleString()} runs</p>
                      <p className="text-xs text-muted-foreground">{formatTimeAgo(wf.createdAt)}</p>
                    </div>
                    <button
                    ...
                    >
                      <span className={cn(
                        ...
                        wf.enabled
                          ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:ring-emerald-300"
                          : "bg-amber-50 text-amber-700 border-amber-200 hover:ring-amber-300"
                      )}>
                        <span className={cn("w-1.5 h-1.5 rounded-full", wf.enabled ? "bg-emerald-500" : "bg-amber-500")} />
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger
                        onClick={(e) => e.stopPropagation()}
                        className="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors outline-none"
                      >
                        <MoreVertical className="w-4 h-4" />
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-40">
                        <DropdownMenuItem onSelect={() => setEditingWorkflow(wf)}>
                          <Edit2 className="w-3.5 h-3.5" /> Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => handleDuplicate(wf)}>
                          <Copy className="w-3.5 h-3.5" /> Duplicate
                        </DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => handleToggle(wf.id)}>
                          {wf.enabled ? <><Pause className="w-3.5 h-3.5" /> Stop</> : <><Play className="w-3.5 h-3.5" /> Activate</>}
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem variant="destructive" onSelect={() => handleDelete(wf.id)}>
